    ...
    case "massUpdate":
      for (var update in massUpdateObject) {
        updateRow(
          db,
          table,
          massUpdateObject[update].update,
          massUpdateObject[update].criteria
        );
      }
      return;
    default:
...
function createInvoice(invoiceObject, usageItemUpdateObject, emailObject) {
  ...
  crud("create", "invoiceTool", "Invoices", {
    updateObject: invoiceObject,
    db,
  });
  crud("massUpdate", "invoiceTool", "Notify Usage Data", {
    massUpdateObject: usageItemUpdateObject,
    db,
  });
...
function addToBulk(invoiceObject, usageItemUpdateObject, newRows) {
  ...
  var exportSheet =
    SpreadsheetApp.openById(notifyBulk).getSheetByName("Tool Export");
  exportSheet
    .getRange(
...
function addNewIds(options) {
  ...
  if (newDepartments) {
    ...
    departmentSheet
      .getRange(
        departmentSheet.getLastRow() + 1,
        1,
        newDepartments.length,
        newDepartments[0].length
      )
      .setValues(newDepartments);
  }
  ...
  if (newServices) {
    ...
    servicesSheet
      .getRange(
        servicesSheet.getLastRow() + 1,
        1,
        newServices.length,
        newServices[0].length
      )
      .setValues(newServices);
  }

  if (notifyUsageIds) {
    var notifyUsageSheet = ss.getSheetByName("Notify Usage Data");
    notifyUsageSheet
      .getRange(2, 14, notifyUsageIds.length, 1)
      .setValues(notifyUsageIds);
  }
}
