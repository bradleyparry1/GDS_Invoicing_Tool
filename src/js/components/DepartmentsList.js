...
function DepartmentsList() {
  ...
  const filterFunction = (filter, row) => {
    if (
      row.DepartmentName.toLowerCase().indexOf(filter.value.toLowerCase()) !==
      -1
    ) {
      return true;
    } else {
      return Object.values(row._original.services).some((service) => {
        if (
          service.ServiceName.toLowerCase().indexOf(
  ...
  const columns = [
    {
      ...
      Cell: (props) => (
        <span onClick={() => viewDepartment(props.original.ID)}>
          {props.value}
        </span>
      ),
      filterable: true,
    ...
    },
    {
      ...
      accessor: (department) =>
        calculateDepartmentUsageBillingTotal(department),
    ...
    },
    {
      ...
      accessor: (department) =>
        calculateDepartmentInvoiceValue(department) +
        calculateDepartmentPrepaidValueUsed(department),
      Cell: (props) => <span>{formatMoney(props.value)}</span>,
      className: "text-center",
    },
    {
      ...
      accessor: (department) =>
        ...
        calculateDepartmentPrepaidValueUsed(department),
      Cell: (props) => (
        <span className={props.value > 0 ? "red" : "green"}>
