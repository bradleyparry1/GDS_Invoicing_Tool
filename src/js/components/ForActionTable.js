...
function ForActionTable(props) {
  ...
  const filterUsageInNeedOfAction = (department) => {
    ...
    return getDepartmentCharateristics(department, "usage")
      .filter((usageItem) => {
        return (
          usageItem.totalcost >
          Number(usageItem.InvoiceAmount ? usageItem.InvoiceAmount : 0) +
            Number(usageItem.PrepaymentAmount ? usageItem.PrepaymentAmount : 0)
        );
      })
      .map((usageItem) => {
        ...
        usageItem.displayInvoiceAmount =
          usageItem.totalcost -
          (usageItem.PrepaymentAmount ? usageItem.PrepaymentAmount : 0);
        ...
        forEach(prepayments, (prepayment) => {
          const serviceIdList = prepayment.ServiceIDs
            ? JSON.parse(prepayment.ServiceIDs)
            : [];
          ...
          if (
            ...
            serviceIdList.includes(usageItem.service_id) &&
            usageItemStart >= prepaymentStart &&
            usageItemEnd <= prepaymentEnd
          ) {
            if (prepayments[prepayment.ID].Remaining > usageItem.totalcost) {
              ...
              usageItem.PrepaymentID = prepayment.ID;
              usageItem.potentialPrepaymentAmount =
                prepayments[prepayment.ID].Remaining;
              prepaid = true;
            }
          }
        });
      ...
      (object, usageItem) => {
        if (poGroups[usageItem.service_id]) {
          ...
          map(poGroups[usageItem.service_id].pos, (poId) => {
            object[poGroups[usageItem.service_id].posJoin].pos[poId] =
              pos[poId];
          });
        } else {
  ...
  const displayGroups = createDisplayGroups(
    usage,
    poGroups,
    servicesWithSamePos
  );
