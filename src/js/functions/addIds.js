...
function addAllIds(parse,existing){
    ...
    map(notifyUsage,(notifyUsageItem,i) => {
        if(notifyUsageItem.organisation_id && !existingDepartments[notifyUsageItem.organisation_id]){
            newDepartments.push(addNewDepartment(notifyUsageItem));
            existingDepartments[notifyUsageItem.organisation_id] = {ID: notifyUsageItem.organisation_id, ProductID: 1, OrganisationName: notifyUsageItem.organisation_name };
        }
        if(!existingServices[notifyUsageItem.service_id]){
            newServices.push(addNewService(notifyUsageItem));
            existingServices[notifyUsageItem.service_id] = {ID: notifyUsageItem.service_id, DepartmentID: notifyUsageItem.organisation_id, ServiceName: notifyUsageItem.service_name };
        }
    ...
    submitNewIds(newDepartments,newServices,notifyUsageIds,updateNotifyUsageIds);

    return { departments: existingDepartments, services: existingServices, notifyUsage };
}

function submitNewIds(newDepartments,newServices,notifyUsageIds,updateNotifyUsageIds) {
    const options = {};
    ...
    google.script.run.withFailureHandler(msg => {
        console.log(msg);
